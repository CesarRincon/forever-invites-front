...
export default function StarshineGlitter() {
    ...
    useEffect(() => {
        ...
        // Cargar imagen del sparkle (PNG base64 del efecto original)
        ...
        const stars = Array.from({ length: STAR_COUNT }).map(() => ({
            ...
            size: [20, 30, 50][Math.floor(Math.random() * 3)],
            time: Math.random() * 6,
        ...
        function animate() {
            ...
            stars.forEach((s) => {
                ...
                const t = (s.time % 6) / 6; // ciclo 0 → 1 → 0
                const scale = t < 0.5 ? t * 2 : (1 - t) * 2;
                const rotation = t * Math.PI * 4;
                const opacity = scale * 0.4;
                ...
                ctx.drawImage(sparkleImg, -s.size / 2, -s.size / 2, s.size, s.size);
                ctx.restore();
            });
